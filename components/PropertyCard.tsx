...
export const PropertyCard: React.FC<Props> = ({ property }) => {
  ...
  return (
    <Link 
      href={`/p/${property.id}`}
      className="block bg-white border border-gray-100 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer group"
    >
      ...
      <div className="p-4">
        ...
        <div className="mt-4 w-full py-2 text-center bg-slate-50 text-slate-700 text-sm font-medium rounded border border-gray-200 group-hover:bg-slate-100 transition-colors">
          View Property
        </div>
      </div>
    </Link>
